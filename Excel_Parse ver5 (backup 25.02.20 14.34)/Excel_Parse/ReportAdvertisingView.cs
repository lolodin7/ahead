...
namespace Excel_Parse
{
    public partial class ReportAdvertisingView : Form 
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (adgroupsInCampaignsMode) { advFix.SetAdgroupsInCampaignsMode(); }
            else if (campaignInProductsMode) { advFix.SetCampaignInProductsMode(); }
            else if (productsInMarketplaces) { advFix.SetProductsInMarketplaces(); }
            else if (targetingInAdgroupsMode) { advFix.SetTargetingInAdgroupsMode(); }

            if (byDays) { advFix.SetByDaysMode(); }
            else if (byWeeks) { advFix.SetByWeeksMode(); }
            else if (byMonths) { advFix.SetByMonthsMode(); }
            else if (byCustom) { advFix.SetByCustomDateMode(); }                    
